# FP.1 - FP.4 Analysis Scripts

This directory contains Python scripts for analyzing the FP.1 (Match 3D Objects), FP.2 (Lidar TTC), FP.3 (Keypoint Match Filtering), and FP.4 (Camera TTC) task results.
...
**Note:** All analysis scripts (FP.2, FP.3, FP.4) support track_id filtering. They automatically read the tracked preceding vehicle's track_id from `output/tracked_preceding_vehicle_track_id.txt` (generated by the C++ executable) to filter results for the preceding vehicle on the ego lane.
