# FP.1 - FP.5 Analysis Scripts

This directory contains Python scripts for analyzing the FP.1 (Match 3D Objects), FP.2 (Lidar TTC), FP.3 (Keypoint Match Filtering), FP.4 (Camera TTC), and FP.5 (Performance Assessment 1) task results.
...
python fp5_detailed_analysis.py  # Run detailed root cause analysis
```
...
| `fp5_analysis.py` | FP.5 | Performance assessment with automated discrepancy detection | `ttc_camera.csv`, `ttc_lidar_comparison.csv` |
| `fp5_detailed_analysis.py` | FP.5 | **Root cause investigation** of lidar TTC outliers with multiple diagnostic analyses | `ttc_camera.csv`, `ttc_lidar_comparison.csv` |
...
```bash
# Run with default settings (looks for output/bb_matches.csv in current directory)
python fp1_analysis.py

# Run without showing interactive plots
python fp1_analysis.py --no-show

# Specify custom CSV path
python fp1_analysis.py --csv output/bb_matches.csv

# Specify output directory for plots
python fp1_analysis.py --csv output/bb_matches.csv --output ./plots

# Use custom detector/descriptor names in plot title
python fp1_analysis.py --csv output/bb_matches.csv --detector SHITOMASI --descriptor ORB
```
...
This CSV is automatically generated by the `3D_object_tracking` executable and saved to `analysis/output/bb_matches.csv` after processing the image sequence.
...
cd build && ./3D_object_tracking

# Then run analysis (from analysis directory)
cd analysis && python fp1_analysis.py
```

This generates a plot showing how the number of matches for the tracked preceding vehicle (track_id=1) changes across frames. With 10% shrink factor, match counts for the preceding vehicle typically range from 77-102.
...
cd build && ./3D_object_tracking
cp output/bb_matches.csv output/bb_matches_shitomasi_orb.csv

# Edit FinalProject_Camera.cpp to use HARRIS + SIFT
# Then recompile and run
cd build && make && ./3D_object_tracking
cp output/bb_matches.csv output/bb_matches_harris_sift.csv

# Analyze both
python fp1_analysis.py --csv output/bb_matches_shitomasi_orb.csv --detector SHITOMASI --descriptor ORB
python fp1_analysis.py --csv output/bb_matches_harris_sift.csv --detector HARRIS --descriptor SIFT
```
...
```bash
python fp1_analysis.py --csv output/bb_matches.csv --all-boxes
```

This generates an additional plot (`output/fp1_all_boxes_SHITOMASI_ORB.png`) showing separate lines for each bounding box.

### Example 4: Batch Analysis for All Combinations

When `bTestAllCombinations = true` in the C++ code, it tests all detector-descriptor pairs and saves results to `analysis/output/detector_descriptor_results.csv`. You can process these results separately.

## Understanding the Results

### Preceding Vehicle Focus

By default, the scripts focus on the tracked preceding vehicle using the **track_id** read from `tracked_preceding_vehicle_track_id.txt`. This file is generated by the C++ executable and identifies the preceding vehicle on the ego lane in the KITTI sequence, which is the most important object for TTC (Time-to-Collision) calculations.
...
├── fp5_analysis.py    # FP.5 analysis script (basic outlier detection)
├── fp5_detailed_analysis.py  # FP.5 detailed root cause investigation
